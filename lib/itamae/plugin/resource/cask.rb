    ...
    module Resource
      class Cask < Itamae::Resource::Base
        ...
        def set_current_attributes
          ...
          result = run_command("/usr/local/bin/brew-cask list | grep '#{attributes.target}$'", error: false)
          current.exist = result.exit_status == 0
        end

        def action_install(options)
          unless current.exist
            run_command(["/usr/local/bin/brew-cask", "install", attributes.target])
          end
        end

        def action_alfred(options)
          run_command(["/usr/local/bin/brew-cask", "alfred", attributes.target])
        end

        private

        def ensure_brew_cask_availability
          if run_command("/usr/local/bin/brew-cask > /dev/null", error: false).exit_status != 0
